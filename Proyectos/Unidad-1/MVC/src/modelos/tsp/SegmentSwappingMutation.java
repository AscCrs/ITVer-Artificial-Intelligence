 ...
 * @author Aaron Foltz
 */
public class SegmentSwappingMutation extends MutationOperator {

  private int m_startOffset = 1;
  private GrafoTSP salesman;
   ...
   */
  public SegmentSwappingMutation() throws InvalidConfigurationException {
    super();
  }

  /**
   * Constructs a new instance of this MutationOperator with the given mutation
   * rate.
   *
   * @param a_config the configuration to use
   * @param a_desiredMutationRate desired rate of mutation, expressed as the
   * denominator of the 1 / X fraction. For example, 1000 would result in 1/1000
   * genes being mutated on average. A mutation rate of zero disables mutation
   * entirely
   * @param salesman An instance of the TSP problem at hand
   * @throws InvalidConfigurationException
   *
   * @author Aaron Foltz
  ...
  /**
   * @param a_config the configuration to use
   * @param salesman an instance of the TSP at hand
   * @throws InvalidConfigurationException
   *
   * @author Klaus Meffert
   * @since 3.0
  ...
  /**
   * Operate on the given array of genes. This method is only called when it is
   * already clear that the mutation must occur under the given mutation rate.
   * It provides a simple 2-Opt Mutation Operator, swapping segments in the
   * array if it is advantageous (in terms of distance)
   *
   * @param a_generator a random number generator that may be needed to perform
   * a mutation
  ...
  protected Gene[] operate(final RandomGenerator a_generator,
    ...
    //CAMBIO
//    if(Math.random()<0.3){
//      List<Gene> lista = new ArrayList();
//      for(Gene x:a_genes){
//        lista.add(x);
//      }
//      Collections.shuffle(lista);
//      Gene[] otro = new Gene[lista.size()];
//      int i=0;
//      for(Gene x:lista){
//        otro[i++]=x;
//      }
//      return otro;
//    }
    int gene1 = (int)(Math.random()*a_genes.length);
    int gene3 = gene1;
    do{
      gene3 = (int)(Math.random()*a_genes.length); 
    } while(gene3==gene1);
    int menor = Math.min(gene1, gene3);
    int mayor = Math.max(gene1, gene3);
    Gene[] otro = Arrays.copyOf(a_genes, a_genes.length);
    int j = menor;
    for (int i = mayor; i >= menor; i--) {
      otro[j++] = a_genes[i];
    }
//      String despues = getVector(otro);
//      for (int i = 0; i < despues.length(); i++) {
//        if (antes.charAt(i) != despues.charAt(i)) {
//          System.out.println("A=" + antes);
//          System.out.println("D=" + despues);
//        }
//      }
    return otro;
    //CAMBIO
...
//    int iGene1 = ((IntegerGene) a_genes[gene1]).intValue();
//    int iGene2 = ((IntegerGene) a_genes[gene2]).intValue();
//    int iGene3 = ((IntegerGene) a_genes[gene3]).intValue();
//    int iGene4 = ((IntegerGene) a_genes[gene4]).intValue();
//    int edge1 = (int) salesman.distancia(iGene1, iGene2);
//    int edge2 = (int) salesman.distancia(iGene3, iGene4);
//
//    // Check the distance between the swapped edges
//    int edge3 = (int) salesman.distancia(iGene1, iGene4);
//    int edge4 = (int) salesman.distancia(iGene2, iGene3);
//
...
//    return a_genes;
  ...
  private String getVector(Gene[] genes) {
    String sal = "";
    for (int i = 0; i < genes.length; i++) {
      sal += genes[i].getAllele() + ",";
    }
    return sal;
  }
}
