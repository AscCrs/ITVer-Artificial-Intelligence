 ...
 * random. My implementation takes a random selection from the remaining "not
 * picked" cities.
 *
...
public class TravelingSalesmanHeuristicCrossover extends BaseGeneticOperator {
  ...
  public TravelingSalesmanHeuristicCrossover(Configuration a_configuration,
    GrafoTSP salesman) throws InvalidConfigurationException {
   ...
   * @since 2.1
   */
  public void operate(final IChromosome a_firstMate,final IChromosome a_secondMate) {
  ...
  protected Gene[] operate(final Gene[] a_g1, final Gene[] a_g2) {
    ...
    while (not_picked.size() > 1) {
      ...
      if (n1 == null) {
        ...
        int iLast = ((IntegerGene)last).intValue();
        int in1 = ((IntegerGene)n1).intValue();
        int in2 = ((IntegerGene)n2).intValue();        
        pick1 = salesman.distancia(iLast, in1) < salesman.distancia(iLast, in2);
      }
      if (pick1) {
        picked = n1;
        other = n2;
